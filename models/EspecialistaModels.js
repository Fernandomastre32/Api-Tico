...
class Especialista {
    static async findAll() {
        const result = await pool.query('SELECT id, nombre, email, especialidad_principal, rol_id, estado_activo FROM especialistas WHERE estado_activo = TRUE');
        return result.rows;
    }
    ...
    static async create(data) {
        ...
        const result = await pool.query(
            'INSERT INTO especialistas (nombre, email, password, especialidad_principal, rol_id) VALUES ($1, $2, $3, $4, $5) RETURNING id, nombre, email, especialidad_principal, rol_id, estado_activo',
    ...
    static async findById(id) {
        const result = await pool.query('SELECT id, nombre, email, especialidad_principal, rol_id, estado_activo FROM especialistas WHERE id = $1', [id]);
        return result.rows[0];
    }
    ...
    static async update(id, data) {
        ...
        const result = await pool.query(
            'UPDATE especialistas SET nombre = $1, email = $2, especialidad_principal = $3, rol_id = $4 WHERE id = $5 RETURNING id, nombre, email, especialidad_principal, rol_id, estado_activo',
